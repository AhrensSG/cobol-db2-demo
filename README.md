...
Proyecto personal de mi formación **autodidacta** en el stack mainframe: **COBOL + SQL embebido en DB2**, **CICS + BMS** (transacciones online en pantallas 3270) y **abends + debugging con XPEDITER**.
...
## Mi experiencia con COBOL (autodidacta)

Me formé por mi cuenta en el stack mainframe. Lo que demuestro en este proyecto:

- **COBOL base + JCL** — divisiones del lenguaje, `PIC`, trabajo con ficheros secuenciales, `COPY` de copybooks y JCL de compilación/ejecución.
- **SQL embebido en DB2** — este proyecto principal: `DECLARE CURSOR`, `OPEN`/`FETCH`/`CLOSE`, comunicación vía `SQLCA` y manejo de resultados por `SQLCODE`.
- **CICS + BMS** — mini aplicación online de cuentas (alta/consulta/baja) en pantallas 3270 — ver `cics-bms/`.
- **Abends y debugging** — reproducción y corrección de `S0C7`/`S322` y depuración con XPEDITER — ver `abend-debug/`.
...
El repo contiene los **tres proyectos que he hecho**:

```
├── src/                  # COBOL + SQL embebido DB2 (consulta de cuentas)
│   ├── copybook/
...
├── cics-bms/             # CICS + BMS (aplicación online 3270)
└── abend-debug/          # Abends (S0C7, S322) + debugging XPEDITER
```
